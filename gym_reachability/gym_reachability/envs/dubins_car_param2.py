...
class DubinsCarParam2Env(gym.Env):
  ...
  def __init__(
    ...
    # Gym variables.
    self.action_space = gym.spaces.Discrete(3) #{-2, 0, 2}
  ...
  def get_value(self, q_func, theta, param, nx=101, ny=101, addBias=False):
    ...
    while not it.finished:
      ...
      y = ys[idx[1]]
      l_x = self.target_margin(np.array([x, y, param]))
      gsc_x = self.safety_margin(np.array([x, y]))
...
# == Plotting Functions ==
  ...
  def plot_v_values(
      self, q_func, theta=np.pi / 2, param = 0, ax=None, fig=None, vmin=-1, vmax=1,
